...
#
### <b>Package linux-aws-scripts-utils-s3Backups CopyRight** ©</b> <img src="https://github.com/RMelanson/linux-scripts-bootstraps/blob/master/images/policeman.png" width="5%" align= "right">
...
- https://github.com/RMelanson/linux-aws-scripts-utils-s3Backups
#
...
- Purpose: s3BackupsBootstrap.sh is a simplified installation package for ...
- Description: s3BackupsBootstrap.sh is a  bootstrap utility for downloading and installing .... XXXXXXXXX.sh may be installed as a bootstrap or after the Linux system has been launched. 
- The s3BackupsBootstrap.sh is used to download ??????? from the git repository for ...
#
...
- Execute the s3BackupsBootstrap.sh*, upon installation of a new Linux installation as a bootstrap or as a stand alone bash script. If executing the script manually on a Linux O.S. after downloading from the git repository, be sure to:
  - execute as root ./s3BackupsBootstrap.sh
  - or as user with sudo access sudo ./s3BackupsBootstrap.sh
- *Notes: 
    - All script installation packages are installed under /tmp/scripts/???/???/??? directory and may be analyzed in that directory.
    - You may modify the script installation file (s3BackupsBootstrap.sh) to change the directory package installation location.
...
- There are two ways to install any of the packages listed under contents as file Format Type (s3BackupsBootstrap.sh).
